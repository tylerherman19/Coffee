...
import { ArrowLeft, ChevronRight, Clock, Coffee, List, LoaderCircle, Map as MapIcon, Search, SlidersHorizontal, Star, Tag, X } from 'lucide-react';
...
export default function Home() {
  ...
  return <div className="site-shell">
    <header className="topbar"><button className="wordmark" onClick={() => setView('shops')} aria-label="Coffee Prices home"><Coffee aria-hidden="true" /><span>Coffee Prices</span></button><div className="metro-toggle" aria-label="Choose a metro"><button className={metro === 'milwaukee' ? 'active' : ''} onClick={() => setMetro('milwaukee')}>Milwaukee</button><button className={metro === 'twin_cities' ? 'active' : ''} onClick={() => setMetro('twin_cities')}>Twin Cities</button></div><div className="topbar-tools">{data.loadedAt && <p className="freshness"><Clock aria-hidden="true" /> Menus checked {new Intl.DateTimeFormat('en-US', { month: 'short', day: 'numeric' }).format(new Date(data.loadedAt))}</p>}<button className={pricedOnly ? 'prices-toggle active' : 'prices-toggle'} aria-pressed={pricedOnly} onClick={() => setPricedOnly(!pricedOnly)}><Tag aria-hidden="true" /> Has prices</button></div></header>
