...
function ShopDetail({ shop, items, onBack }: { shop: Shop; items: Item[]; onBack: () => void }) {
  ...
  return <main className="detail-shell">
    ...
    <div className="detail-address">{shop.address || 'Address unavailable'} · {shop.opening_hours || 'Hours unavailable'}{checked ? ` · Menu checked ${new Intl.DateTimeFormat('en-US', { month: 'short', day: 'numeric' }).format(new Date(checked))}${RENDERED_AT - new Date(checked).getTime() > STALE_AFTER_MS ? ' (stale)' : ''}` : ''}</div>
    {Object.keys(groups).length === 0 ? <div className="empty"><Coffee aria-hidden="true" /><h2>Menu collection is pending</h2><p>The shop is mapped. Its direct menu source has not been collected yet.</p></div> : Object.entries(groups).map(([group, entries]) => <section className="menu-section" key={group}><h2>{group}</h2>{entries.sort((a, b) => a.name.localeCompare(b.name)).map((item) => <div className="menu-row" key={item.id}><div><h3>{item.name}</h3><p>{item.size_label || (item.size_oz ? `${item.size_oz} oz` : 'Size not listed')}{item.size_confidence === 'inferred' ? ' · estimated size' : ''}</p></div><span className="leader" aria-hidden="true"></span><div className="menu-price"><strong>{formatPrice(item.current_price_cents)}</strong>{item.current_price_cents && item.size_oz ? <small>{item.size_confidence === 'inferred' ? '~' : ''}{money.format(item.current_price_cents / 100 / item.size_oz)}/oz</small> : null}</div></div>)}</section>)}
...
export default function Home() {
  ...
  const [drink, setDrink] = useState('caramel_latte');
  ...
  if (selectedShop) return <ShopDetail shop={selectedShop} items={data.items} onBack={() => { setSelectedShop(null); requestAnimationFrame(() => window.scrollTo(0, scrollRef.current)); }} />;
  return <div className="site-shell">
    <header className="topbar"><button className="wordmark" onClick={() => setView('compare')} aria-label="Coffee Prices home"><Coffee aria-hidden="true" /><span>Coffee Prices</span></button><div className="metro-toggle" aria-label="Choose a metro"><button className={metro === 'milwaukee' ? 'active' : ''} onClick={() => setMetro('milwaukee')}>Milwaukee</button><button className={metro === 'twin_cities' ? 'active' : ''} onClick={() => setMetro('twin_cities')}>Twin Cities</button></div><div className="topbar-tools"><button className={pricedOnly ? 'prices-toggle active' : 'prices-toggle'} aria-pressed={pricedOnly} onClick={() => setPricedOnly(!pricedOnly)}><Tag aria-hidden="true" /> Has prices</button></div></header>
    <nav className="desktop-nav" aria-label="Main navigation"><button className={view === 'compare' ? 'active' : ''} onClick={() => setView('compare')}>Compare</button><button className={view === 'shops' ? 'active' : ''} onClick={() => setView('shops')}>Shops</button><button className={view === 'map' ? 'active' : ''} onClick={() => setView('map')}>Map</button></nav>
    {view === 'shops' && <main className="content-shell"><section className="section-heading"><div><p>{metro === 'milwaukee' ? 'Milwaukee' : 'Minneapolis–Saint Paul'}</p><h1>What does coffee cost today?</h1></div><span>{loading ? 'Pulling menus…' : pricedOnly || query || openOnly ? `${visibleShops.length} shown of ${metroShops.length}` : `${metroShops.length} shops mapped`}</span></section><div className="filters"><label className="search"><Search aria-hidden="true" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search shops or neighborhoods" /></label><button className={openOnly ? 'filter-button active' : 'filter-button'} onClick={() => setOpenOnly(!openOnly)}><SlidersHorizontal aria-hidden="true" /> Open now</button></div>{hoods.length > 1 && <div className="hood-filter"><select className="hood-select" value={activeHood} onChange={(event) => setHood(event.target.value)} aria-label="Filter by neighborhood"><option value="">All neighborhoods</option>{hoods.map(([name, count]) => <option key={name} value={name}>{name} ({count})</option>)}</select></div>}{loading ? <div className="loading-list"><LoaderCircle aria-hidden="true" /> Pulling the latest menus…</div> : error ? <div className="error-state">{error}</div> : visibleShops.length ? <div className="shop-list">{visibleShops.map((shop) => <ShopRow key={shop.id} shop={shop} items={data.items} dimmed={openOnly && isOpenNow(shop.opening_hours) == null} onOpen={() => { scrollRef.current = window.scrollY; setSelectedShop(shop); }} />)}</div> : <div className="empty"><Coffee aria-hidden="true" /><h2>No shops match</h2><p>Clear the filters or check back after the next menu pull.</p></div>}</main>}
    {view === 'compare' && <main className="content-shell compare-shell"><section className="section-heading"><div><p>Same drink, fair comparison</p><h1>Find your cup</h1></div><span>{loading ? 'Pulling menus…' : `${ranked.length} menu matches`}</span></section><div className="drink-scroll" aria-label="Drink type">{(drinkTypes.length ? drinkTypes : ['caramel_latte', 'latte', 'cappuccino', 'espresso', 'drip', 'cold_brew']).map((type) => <button key={type} className={drink === type ? 'active' : ''} onClick={() => setDrink(type)}>{drinkLabels[type] || type.replaceAll('_', ' ')}</button>)}</div>{hoods.length > 1 && <div className="hood-filter"><select className="hood-select" value={activeHood} onChange={(event) => setHood(event.target.value)} aria-label="Filter by neighborhood"><option value="">All neighborhoods</option>{hoods.map(([name, count]) => <option key={name} value={name}>{name} ({count})</option>)}</select></div>}<div className="compare-note">Prices use direct shop menus. A tilde marks an inferred serving size.</div>{loading ? <div className="loading-list"><LoaderCircle aria-hidden="true" /> Pulling the latest menus…</div> : error ? <div className="error-state">{error}</div> : <div className="rank-list">{(showAll ? ranked : ranked.slice(0, 75)).map(({ item, shop, price, locations }, index) => <button className="rank-row" key={item.id} onClick={() => { scrollRef.current = window.scrollY; setSelectedShop(shop); }}><span className="rank-number">{index + 1}</span><span className="rank-main"><strong>{shop.name}</strong><small>{item.name} · {locations > 1 ? `${locations} locations · chain menu` : neighborhood(shop)}</small><Rating value={shop.rating} count={shop.review_count} /></span><span className="rank-price"><strong>{formatPrice(price)}</strong>{item.size_oz ? <small>{item.size_confidence === 'inferred' ? '~' : ''}{money.format(price / 100 / item.size_oz)}/oz</small> : <small>size unlisted</small>}</span></button>)}{!comparisons.length && <div className="empty"><Coffee aria-hidden="true" /><h2>No comparable prices yet</h2><p>This fills in as direct menus are collected.</p></div>}{!showAll && ranked.length > 75 && <button className="show-more" onClick={() => setShowAllKey(rankingKey)}>Show all {ranked.length} matches</button>}{showAll && ranked.length > 75 && <button className="show-more" onClick={() => setShowAllKey(null)}>Show top 75</button>}</div>}</main>}
    {view === 'map' && <main className="map-shell"><div className="map-caption"><div><p>{metro === 'milwaukee' ? 'Milwaukee' : 'Twin Cities'}</p><h1>Every cup on the map</h1></div><span>{visibleShops.length === metroShops.length ? `${visibleShops.length} locations` : `${visibleShops.length} of ${metroShops.length} locations`}</span></div>{(query || openOnly) && <div className="map-filter-note"><span>Filtered by {[query && `“${query}”`, openOnly && 'open now'].filter(Boolean).join(' · ')}</span><button onClick={() => { setQuery(''); setOpenOnly(false); }}><X aria-hidden="true" /> Clear</button></div>}{hoods.length > 1 && <div className="hood-filter map-hood"><select className="hood-select" value={activeHood} onChange={(event) => setHood(event.target.value)} aria-label="Filter by neighborhood"><option value="">All neighborhoods</option>{hoods.map(([name, count]) => <option key={name} value={name}>{name} ({count})</option>)}</select></div>}<MapView shops={visibleShops} items={data.items} metro={metro} onOpen={setSelectedShop} /></main>}
