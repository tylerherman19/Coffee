...
const drinkOrder = ['latte', 'caramel_latte', 'cold_brew', 'drip', 'cappuccino', 'americano', 'espresso', 'macchiato', 'mocha', 'chai', 'tea', 'other'];
...
function ShopDetail({ shop, items, onBack }: { shop: Shop; items: Item[]; onBack: () => void }) {
  ...
  return <main className="detail-shell">
    ...
    <div className="detail-address">{shop.address || 'Address unavailable'} · {shop.opening_hours || 'Hours unavailable'}{checked ? ` · Menu checked ${new Intl.DateTimeFormat('en-US', { month: 'short', day: 'numeric' }).format(new Date(checked))}${Date.now() - new Date(checked).getTime() > 60 * 864e5 ? ' (stale)' : ''}` : ''}</div>
