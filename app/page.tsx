...
import { ArrowLeft, ChevronRight, Coffee, List, LoaderCircle, Map as MapIcon, Search, SlidersHorizontal, Star, Tag } from 'lucide-react';
import { drinkLabels, loadCoffeeData, shopDrink, type CoffeeData, type Item, type Shop } from '@/lib/coffee-data';
...
function ShopDetail({ shop, items, onBack }: { shop: Shop; items: Item[]; onBack: () => void }) {
  ...
  return <main className="detail-shell">
    ...
    <div className="detail-address">{shop.address || 'Address unavailable'} · {shop.opening_hours || 'Hours unavailable'}{checked ? ` · Menu checked ${new Intl.DateTimeFormat('en-US', { month: 'short', day: 'numeric' }).format(new Date(checked))}` : ''}</div>
...
export default function Home() {
  ...
  useEffect(() => { setShowAll(false); }, [drink, activeHood, metro]);
  ...
  const comparisons = useMemo(() => data.items.filter((item) => item.drink_type === drink && item.current_price_cents != null).map((item) => ({ item, shop: shopsById.get(item.shop_id), price: item.current_price_cents as number })).filter((entry): entry is { item: Item; shop: Shop; price: number } => Boolean(entry.shop && entry.shop.metro === metro && (!activeHood || entry.shop.neighborhood === activeHood))).sort((a, b) => a.price - b.price), [data.items, shopsById, drink, metro, activeHood]);
  ...
  return <div className="site-shell">
    ...
    {view === 'compare' && <main className="content-shell compare-shell"><section className="section-heading"><div><p>Same drink, fair comparison</p><h1>Find your cup</h1></div><span>{loading ? 'Pulling menus…' : `${comparisons.length} menu matches`}</span></section><div className="drink-scroll" aria-label="Drink type">{(drinkTypes.length ? drinkTypes : ['caramel_latte', 'latte', 'cappuccino', 'espresso', 'drip', 'cold_brew']).map((type) => <button key={type} className={drink === type ? 'active' : ''} onClick={() => setDrink(type)}>{drinkLabels[type] || type.replaceAll('_', ' ')}</button>)}</div>{hoods.length > 1 && <div className="hood-filter"><select className="hood-select" value={activeHood} onChange={(event) => setHood(event.target.value)} aria-label="Filter by neighborhood"><option value="">All neighborhoods</option>{hoods.map(([name, count]) => <option key={name} value={name}>{name} ({count})</option>)}</select></div>}<div className="compare-note">Prices use direct shop menus. A tilde marks an inferred serving size.</div>{loading ? <div className="loading-list"><LoaderCircle aria-hidden="true" /> Pulling the latest menus…</div> : error ? <div className="error-state">{error}</div> : <div className="rank-list">{(showAll ? comparisons : comparisons.slice(0, 75)).map(({ item, shop, price }, index) => <button className="rank-row" key={item.id} onClick={() => { scrollRef.current = window.scrollY; setSelectedShop(shop); }}><span className="rank-number">{index + 1}</span><span className="rank-main"><strong>{shop.name}</strong><small>{item.name} · {neighborhood(shop)}</small><Rating value={shop.rating} count={shop.review_count} /></span><span className="rank-price"><strong>{formatPrice(price)}</strong>{item.size_oz ? <small>{item.size_confidence === 'inferred' ? '~' : ''}{money.format(price / 100 / item.size_oz)}/oz</small> : <small>size unlisted</small>}</span></button>)}{!comparisons.length && <div className="empty"><Coffee aria-hidden="true" /><h2>No comparable prices yet</h2><p>This fills in as direct menus are collected.</p></div>}{!showAll && comparisons.length > 75 && <button className="show-more" onClick={() => setShowAll(true)}>Show all {comparisons.length} matches</button>}{showAll && comparisons.length > 75 && <button className="show-more" onClick={() => setShowAll(false)}>Show top 75</button>}</div>}</main>}
    {view === 'map' && <main className="map-shell"><div className="map-caption"><div><p>{metro === 'milwaukee' ? 'Milwaukee' : 'Twin Cities'}</p><h1>Every cup on the map</h1></div><span>{visibleShops.length} locations</span></div>{hoods.length > 1 && <div className="hood-filter map-hood"><select className="hood-select" value={activeHood} onChange={(event) => setHood(event.target.value)} aria-label="Filter by neighborhood"><option value="">All neighborhoods</option>{hoods.map(([name, count]) => <option key={name} value={name}>{name} ({count})</option>)}</select></div>}<MapView shops={visibleShops} items={data.items} metro={metro} onOpen={setSelectedShop} /></main>}
